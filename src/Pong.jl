...
# The game logic, living within a B+ context (a.k.a. OpenGL window):
function run_game(context::Bplus.GL.Context)
    window::GLFW.Window = context.window

    # Op into some B+ "services".
    # Services are tied to the context.
    # Because the context is a thread-local singleton, we usually
    ...
    # Helper function to read both inputs:
    ...
    while !GLFW.WindowShouldClose(window)
        ...
        # Cap the framerate.
        wait_time = FPS_CAP - delta_seconds
        if wait_time >= 0.001
            sleep(wait_time)
        end
        ...
        p2_pos += BAT_SPEED_PIXELS_PER_SECOND * delta_seconds *
                    v2f(0, sample_p2_input())
        #   2. Stop the bats from leaving the level:
        ...
        @set! p2_pos.y = clamp(p2_pos.y, HALF_BAT_HEIGHT,
                                         LEVEL_SIZE_PIXELS.y - HALF_BAT_HEIGHT)
        #   3. Process ball movement:
        #        (note: the Setfield package and '@set!' macro helps us make copies of immutable data)
        ball_pos += ball_velocity * delta_seconds
        ...
        #   4. Check for collisions between the ball and bats:
        ...
        # Render.
        GL.render_clear(context, GL.Ptr_Target(), vRGBAf(0, 0, 0, 0))
        GL.render_clear(context, GL.Ptr_Target(), @f32(1)) # Clears the depth buffer
        # There are three objects to draw: P1, P2, and the ball.
        render_tasks::Tuple = (
            # Each element is a tuple of (position, size, color, shader).
            (
                p1_pos,
                BAT_SIZE,
        ...
        for (pos, size, color, shader) in render_tasks
            ...
            GL.render_mesh(service_resources.quad, shader)
        end
